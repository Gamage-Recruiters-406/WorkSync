...
const ManageLeaves = () => {
  ...
  const fetchLeaves = async () => {
    try {
      ...
      console.error(err);
      setAlert({ open: true, message: "Failed to fetch leaves", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setAlert({
        open: true,
        message: `Leave ${actionType === "approved" ? "approved" : "rejected"} and email sent successfully!`,
        type: "success",
      });
    } catch (err) {
      setAlert({
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 p-6 space-y-6 bg-gray-50 overflow-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card title="Total Leave Requests" value={total} />
        ...
        <div className="bg-white rounded-xl shadow p-4 overflow-auto">
          {loading ? (
            ...
            <table className="w-full text-sm border border-gray-300 border-collapse min-w-[900px]">
              <thead className="bg-[#087990] text-white">
                <tr>
                  <th className="border px-2 py-3">User</th>
              ...
              <tbody className="bg-gray-100">
                {leaves.map((l) => (
                  <tr key={l._id} className="text-center hover:bg-gray-200">
                    <td className="border px-2 py-2">{l.requestedBy?.fullName || "N/A"}</td>
                      ...
                      {l.sts === "pending" ? (
                        <div className="flex justify-center gap-3">
                          <button
                            ...
                            className="w-9 h-9 rounded-full bg-green-100 text-green-600 hover:bg-green-500 hover:text-white transition"
                          >
                            ✓
                          </button>
                          <button
                            ...
                            className="w-9 h-9 rounded-full bg-red-100 text-red-600 hover:bg-red-500 hover:text-white transition"
                          >
                            ✕
                          </button>
                        </div>
      ...
      {showModal && selectedLeave && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-md rounded-xl shadow-lg p-6">
            ...
            <p className={`text-sm font-medium mb-3 ${actionType === "approved" ? "text-green-600" : "text-red-600"}`}>
              Status will be marked as{" "}
              <span className="uppercase">{actionType === "approved" ? "APPROVED" : "REJECTED"}</span>
            </p>
            <div className="bg-gray-50 border rounded-lg p-3 mb-4 text-sm space-y-1">
              <p><span className="font-medium">Employee:</span> {selectedLeave.requestedBy?.fullName}</p>
              <p><span className="font-medium">Email:</span> {selectedLeave.requestedBy?.email}</p>
              <p><span className="font-medium">Leave Type:</span> {selectedLeave.leaveType}</p>
              <p><span className="font-medium">Duration:</span> {selectedLeave.startDate?.slice(0, 10)} / {selectedLeave.endDate?.slice(0, 10)}</p>
            </div>
            <textarea
              rows="5"
              value={emailMessage}
              onChange={(e) => setEmailMessage(e.target.value)}
              placeholder="Type email message to employee..."
              className="w-full border rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#087990]"
            />
...
const Card = ({ title, value }) => (
  <div className="bg-white p-4 rounded-3xl border shadow text-center">
    ...
    <h2 className="bg-[#087990] w-12 h-12 mx-auto rounded-xl text-2xl font-bold text-white flex items-center justify-center mt-2">{value}</h2>
  </div>
);
