...
// -------------------- Utilities --------------------
const safeGetApiKey = () => {
  // Try multiple places (CRA, Vite, window) to be robust in different setups
  try {
    if (typeof process !== "undefined" && process.env && process.env.REACT_APP_CALENDARIFIC_API_KEY) {
      return process.env.REACT_APP_CALENDARIFIC_API_KEY;
    }
  } catch (e) {}
  try {
    if (typeof import.meta !== "undefined" && import.meta.env && import.meta.env.VITE_CALENDARIFIC_API_KEY) {
      return import.meta.env.VITE_CALENDARIFIC_API_KEY;
    }
  } catch (e) {}
  if (typeof window !== "undefined" && window.REACT_APP_CALENDARIFIC_API_KEY) {
    return window.REACT_APP_CALENDARIFIC_API_KEY;
  }
  return "";
};

// Avoid timezone offset issues when formatting dates for comparison
...
const LeaveRequest = () => {
  const [formData, setFormData] = useState({
    leaveType: "",
    startDate: null, // Date objects for DatePicker
    endDate: null,
    reason: "",
  });
  ...
  // Holidays from Calendarific
  const [holidays, setHolidays] = useState([]); // [{date: "2026-05-23", name: "Vesak"...}, ...]

  const API_KEY = safeGetApiKey() || "BBwGFpeRPxaz3Zty24HFDR0gcuIaP6bE";


  console.log("API_KEY is:", API_KEY);

  // Fetch holidays (frontend-only)
  useEffect(() => {
    const fetchHolidays = async () => {
      if (!API_KEY) {
        console.warn("Calendarific API key not found. Holidays will not be loaded.");
        return;
      }
    ...
    fetchHolidays();
    // re-run if API key or year would change (rare)
  }, [API_KEY]);
  ...
  const getHolidayName = (date) => {
    if (!date) return null;
    const iso = formatDateISO(date);
    return holidays.find((h) => h.date === iso)?.name || null;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.endDate < formData.startDate) {
      setAlert({ open: true, message: "End date cannot be before start date", type: "error" });
      return;
    }

    // Convert to YYYY-MM-DD strings for backend
    const payload = {
      ...formData,
    ...
    try {
      if (editingLeaveId) {
        const res = await api.put(`/updateLeave/${editingLeaveId}`, payload);
        setAlert({ open: true, message: res.data?.message || "Leave updated successfully", type: "success" });
        setEditingLeaveId(null);
      } else {
        const res = await api.post("/addLeave", payload);
  ...
  return (
    <div className="flex h-screen">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <DatePicker
                      ...
                      className="w-full border rounded-lg p-2 mt-2 text-sm"
                      dayClassName={(date) => (isHoliday(date) ? "react-datepicker__day--holiday" : undefined)}
                    />
                    {getHolidayName(formData.startDate) && (
                      <p className="text-xs text-red-600 mt-1">{getHolidayName(formData.startDate)}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <DatePicker
                      ...
                      dayClassName={(date) => (isHoliday(date) ? "react-datepicker__day--holiday" : undefined)}
                    />
                    {getHolidayName(formData.endDate) && (
                      <p className="text-xs text-red-600 mt-1">{getHolidayName(formData.endDate)}</p>
                    )}
                  </div>
                </div>
            ...
            {leaveBalance && (
              <div className="bg-white rounded-2xl shadow p-4 border border-gray-200 max-w-md">
                ...
                {["annual", "casual", "sick"].map((type, idx) => {
                  ...
                  const total = Number(leaveBalance.policy?.[type] ?? 0);
                  ...
                  const color = type === "annual" ? "bg-blue-600" : type === "casual" ? "bg-green-600" : "bg-purple-600";

                  return (
                    <div key={idx} className="mb-6">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="font-medium text-gray-700">{type.charAt(0).toUpperCase() + type.slice(1)} Leave</span>
                        <span className="font-semibold">{display}</span>
                      </div>
                      <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden shadow-inner">
                        <div className={`${color} h-4 rounded-full transition-all duration-500 ease-out`} style={{ width: percent }}></div>
                      </div>
                    </div>
                  );
                })}
                ...
                <div className="flex justify-around text-center">
                  <div className="flex flex-col items-center">
                    <span className="px-3 py-1 bg-[#E6F4F6] text-[#087990] rounded-full font-semibold">{statusCounts.approved ?? 0}</span>
                    <span className="text-gray-500 text-sm mt-1">Approved</span>
                  </div>
                  <div className="flex flex-col items-center">
                    <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full font-semibold">{statusCounts.pending ?? 0}</span>
                    <span className="text-gray-500 text-sm mt-1">Pending</span>
                  </div>
                  <div className="flex flex-col items-center">
                    <span className="px-3 py-1 bg-red-100 text-red-700 rounded-full font-semibold">{statusCounts.rejected ?? 0}</span>
                  ...
                  {leaveHistory.length > 0 ? (
                    leaveHistory.map((leave) => (
                      <tr key={leave._id} className="hover:bg-gray-200 transition-colors duration-150 text-sm">
                        ...
                        <td className="text-center border-r border-white whitespace-nowrap">{leave.startDate ? new Date(leave.startDate).toLocaleDateString() : "-"}</td>
                        <td className="text-center border-r border-white whitespace-nowrap">{leave.endDate ? new Date(leave.endDate).toLocaleDateString() : "-"}</td>
                        ...
                        <td className="text-center border-r border-white whitespace-nowrap">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${leave.sts === "approved" ? "bg-[#E6F4F6] text-[#087990]" : leave.sts === "rejected" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}>
                        ...
                        <td className="text-center border-r border-white flex justify-center gap-2 items-center py-3 whitespace-nowrap">
                          <button onClick={() => handleEdit(leave)} className="p-1 text-blue-600 hover:text-blue-800 hover:bg-blue-100 rounded transition-colors duration-150">
                            <Edit2 size={18} />
                          </button>
                          <button onClick={() => handleDeleteClick(leave._id)} className="p-1 text-red-600 hover:text-red-800 hover:bg-red-100 rounded transition-colors duration-150">
                            <Trash2 size={18} />
                          </button>
                        </td>
                      </tr>
      ...
      {/* ---------------- Modals ---------------- */}
      {alert.open && <CustomAlert message={alert.message} type={alert.type} onClose={() => setAlert({ open: false, message: "", type: "success" })} />}

      {confirmDelete.open && <ConfirmModal message="Are you sure you want to delete this leave request? This action cannot be undone." onConfirm={handleDeleteConfirm} onCancel={() => setConfirmDelete({ open: false, leaveId: null })} />}
    </div>
  );
};
